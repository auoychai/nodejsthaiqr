...
const self = {
    ...
    genQRCode: async (data) => {
        ...
        try {
            ...
            if (vMCInfo.status == true) {
                ...
                await self.storeQRReqInfo(data, vMCInfo);
    ...
    getT60TermTxnProxyId: async (mcInfo, qrConfig) => {
        ...
        try{
            ...
            txnProxyId = term.termId + (espUtil.strRepeat('0', (13 - strQrProxySeq.length))) + strQrProxySeq;
            //logger.debug('txnProxyId:',txnProxyId);
            //logger.debug('Observed TerminalInfo-1:',vMCInfo.data.terminal);
            termId_len = '';
            //v_termId = vMCInfo.data.terminal.termId+txnProxyId;
            v_termId = txnProxyId;
            term.txnProxyId = v_termId;

            logger.debug('Terminal+txnProxyId:',txnProxyId);

            termId_len = espUtil.len_padding(v_termId);
            v_t62Value = qrConf.t62.v_termid.id + termId_len + v_termId;
            ...
            //logger.debug('v_termId:',v_termId);
    ...
    storeQRReqInfo: async (data, vMCInfo) => {
        ...
        /*
        logger.debug('mc-storeQRReqInfo:',mc);
        logger.debug('sto-storeQRReqInfo:',sto);
        logger.debug('term-storeQRReqInfo:',term);
        */
        ...
        try{
            let qrPaidInfo = await new qrPInfo.QRPaidLog(xqrInfo).save();
        }catch(err){
